/*!
 * ShareThis Widget Version 3.8.0-rc10
 * 5/15/09 ShareThis.com 
 */
...
var Widget = new Class({ Implements: Events,
	...
	signIn: function() {
		...
		if(!err_set){
			this.user.signIn($('textAuthUsername').value,$('textAuthPassword').value);
		}
		else{
	...
	postBlogger: function() {
		...
		if(err_set){
			...
			if(!blogid) {
				data="username="+username+"&password="+password+"&d="+glo_content+atag+"&draft="+glo_bloggerDraft+"&t="+glo_title+"&return=json&rememberme="+rememberme+"&forgetme="+forgetme;
			}
			else{
					data="username="+username+"&password="+password+"&d="+glo_content+atag+"&draft="+glo_bloggerDraft+"&t="+glo_title+"&return=json&blogid="+blogid+"&rememberme="+rememberme+"&forgetme="+forgetme;
			}
		...
		if(resp.status==="FAILURE"){
			if(resp.errorMessage && resp.errorMessage.toLowerCase()==="auth_failed") {
				...
				this.fireEvent('postToServiceNeedsMoreInfo', [
					'blogger', 
					'You have successfully signed on. Please select a blog.',
					resp.data
				]);
			}
			else {
	...
	postLive_journal: function(){ 
		...
		if(err_set){
			...
			var data="";
			data="username="+username+"&password="+password+"&d="+comment+"<br/><br/>"+glo_content+atag+"&t="+glo_title+"&return=json&rememberme="+rememberme+"&forgetme="+forgetme;
			var request=new Request({
				method: "post",
	...
	postTwitter: function() { 
		...
		if(!status){
			err_set=true;	
			err+="Please enter a status update.\n";	
		}
		if(status.length > 140){
			err_set=true;	
			err+="Twitter post too long. Please shorten to less than 140 characters.\n";	
		}
		if(err_set){
			widget.validationFailed(err);
		}
	...
	postTwitter_onSuccess: function(responseText, responseXML) {
		...
		if (resp.status && resp.status.toLowerCase() == 'success') {
			...
			if (resp.errorMessage) {
				switch(resp.errorMessage.toLowerCase()) {
					case 'auth_failed':
						this.fireEvent('postToServiceFailed', [
							'twitter', 
							'Invalid Username or Password'
	...
	{ 
		...
		if(err_set){
			...
			var data="";
			data="username="+username+"&password="+password+"&d="+glo_content+atag+"&t="+glo_title+"&return=json&url="+url+"&rememberme="+rememberme+"&forgetme="+forgetme;
	...
	postWordpress_onSuccess: function(responseText, responseXML) {
		...
		if(resp.status.toLowerCase()==="success"){
			...
			if (resp.errorMessage && resp.errorMessage.toLowerCase()==="connection_failed") {
				...
				else if(resp.statusMessage && (resp.statusMessage.toLowerCase()=="post_failed" || resp.statusMessage.toLowerCase() == 'post_wordpress_service_failed')){
					this.fireEvent('postToServiceFailed', [
						'wordpress',
						'Post Failed.'
					]);
	...
	importContacts: function(service, username, password, contact_url, delt) {		
		...
		var data = "provider=" + service.protocolName + "&username=" + username + "&password=" + password + "&contact_url=" + contact_url
...
if (!window.console || !console.firebug) {
		...
		if(str!==""){
			...
			if(selectedType==="ALL"){
				...
				if(isPhone(str)){
					...
					if(!blur){alert("Not a valid Phone Number");}
					type="";
				...
				if(isAIM(str)){
					...
					if(!blur){alert("Not a valid AIM Screen Name");}
					type="";
					document.getElementById("txtTo").value="";			
				}
			}
	...
	function register_OnSuccess(responseText, responseXML){
		...
		if(resp.status==="SUCCESS"){
			...
			widget.fireEvent('registerUserFailed', resp.errorMessage);
		}
	...
	pages: {
		home: {
			...
			showTwitterMenu: function() {
				...
				var f = null;
				f = (function(event) {
					if (event.target != $('twitter_direct_message')
						&& event.target != $('twitter_update_status')
						&& event.target != $('post_twitter_link')
						) {
		...
		sharebox: {
			...
			onReady: function() {
				...
				widget.addEvent('saveToShareBoxSucceeded', function() {
					...
					$('done_message').set('text', 'This item was successfully saved to your ShareBox.');
				});
				widget.addEvent('saveToShareBoxFailed', function() {
					widget.pushModalErrorSheet('Could not save to your ShareBox.');
				});
			},
			onShow: function() {
				if (this.firstShow) {
					// see note in bindHelpText
					this.bindHelpText($('sharebox_tags'), 'Separated by commas');
				}
		...
		register: {
			...
			onReady: function() {
				...
				widget.addEvent('registerUserRequested', function() {
					widget.pushModalWorkingSheet('Creating your account&hellip;');
				});
		...
		send: {
			...
			hasAIMRecipients: function() {
				var retval = false;
				var recipients = widget.user.getSelectedContacts();
				recipients.each(function(contact) {
					if ( (contact.service && contact.service.toLowerCase() == 'aim')
						||
						 (contact.type && contact.type.toLowerCase() == 'aim')
						) {
						retval = true;
					}
				});
			...
			onShow: function() {
				if (this.firstShow) {
					// see note in bindHelpText
					this.bindHelpText($('txtYourAddr'), 'Your Email Address');
				}
			...
			onReady: function() {
				...
				$('btnShareSend').addEvent('click', (function(){
					...
					if(recipients.length<1){
						...
						alert("Please enter less than 25 recipients");
					}
				}).bind(this));
		...
		addressbook: {
			...
			onHide: function() {
				$('done_message').set('text', 'Your message was successfully shared!');
			}
		},
		post: {
			id: 'post_page',
...
Widget.implement({
	services: {
		...
		facebook: {
			title: 'Facebook',
			submitUrl: 'http://www.facebook.com/share.php?u={url}&t={title}',
...
//			dontUseSharURL: 'Shar URLs are not allowed'
		...
		windows_live: {
			...
			onClick: function(event, compositedURL) {
				window.open(compositedURL, 'windowsLivePopup');
				event.stop();
			}*/
		},
		wordpress:  {
		...
		yahoo_myweb: {
			title: 'Y! MyWeb',
			submitUrl: 'http://myweb2.search.yahoo.com/myresults/bookmarklet?u={url}&t={title}',
			destination: 'myweb2.search.yahoo.com'
		},
		*/
		ybuzz: {
			title: 'Buzz Up!',
...
Widget.Contact = new Class({ Implements: Events,
	service: '',
	name: '',
	address: '',
	id: '',
	...
	setAddress: function(text) {
		...
		/*
		if (this.inventName) {
			this.setName(Widget.Contact.generateNameFromAddress(text));
		}
		*/
	},
...
Widget.Contact.guessServiceForAddress = function(address, serviceSrcHint) {
	...
	// if no source hint
	/*
	if (address.toLowerCase().indexOf('@gmail.com') != -1) {
		return 'gmail';
	}
	else if (address.toLowerCase().indexOf('@aol.com') != -1) {
		return 'aol';
	}
	else if (address.toLowerCase().indexOf('@yahoo.com') != -1) {
		return 'yahoo';
	}
	else if (address.toLowerCase().indexOf('@hotmail.com') != -1) {
		return 'msn';
	}
	*/
...
Widget.Carousel = new Class({ Implements: Events,
...
Widget.User = new Class({ Implements: Events,
	email: '',
	name: '',
	authToken: '',
	userID: '',
...
Widget.MirrorContact = new Class({ Extends: Widget.Contact,
	added: false,
...
Widget.ToField = new Class({ Implements: Events,
	...
	_search: function(text) {
		...
		if (this.searchResults && this.searchResults.length) {
			if (this.searchResults.length == 1) {
				// if we have an exact match, don't show the mirror item
				if (this.searchResults[0].address.toLowerCase() != this.searchText.toLowerCase()) {
	...
	*/
	...
	_createToken: function(contact, method) {
		...
		/*if (method == Widget.ToField.createToken_asHTML) {
			return '\
				<a class="token" href="javascript:void(0);" >\
					<span title= "' + contact.name + '<' + contact.address + '>' + '" class="' + (contact.service.length ? contact.service : 'email') + '">\
						' + widget.truncateText(contact.name, 130) + '\
					</span>\
				</a>\
			';
		}
			...
			var name = new Element('span', { 
				...
				'class': contact.service.length ? contact.service : 'email' 
			});
			name.set('html', widget.truncateText(contact.name, 130));
			token.grab(name);
			this._attachTokenBehavior(token, contact);
		}
		return token;*/
	},
	...
	_attachTokenBehavior: function(element, contact) {
		...
		element.addEvent('mouseleave', (function(event) {
			...
			//element.removeEvent('mousemove', mouseMoveHandler);
		}).bind(this));
		*/
		
		/*
		element.addEvent('selected', (function(isSelected) {
			if (isSelected) {
				this.selectedTokens.include(element);
			}
			else {
				this.selectedTokens.erase(element);
			}
		}).bind(this));
		element.getElement('span.deselect_button').addEvent('click', function(event) {
			contact.deselect();
			event.stop();
		});
		*/
