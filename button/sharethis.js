...
ST_JSON = new function() {
  ...
  try{
	...
	if (!SHARETHIS) {
		...
		var SHARETHIS = null;
		/*
		 * parseQueryString.js - a function to parse and decode query strings -- The
		 ...
		 * public domain. Because this program is released into the public domain,
		 * it comes with no warranty either expressed or implied, to the extent
		 * permitted by law. For more public domain JavaScript code by the same
		 * author, visit: http://www.safalra.com/web-design/javascript/
		 */
		function parseQueryString(G) {
			var E = {};
			if (G == undefined) {
				G = location.search ? location.search : ""
			}
			if (G.charAt(0) == "?") {
				G = G.substring(1)
			}
			...
			for ( var C = 0; C < B.length; C++) {
				...
				var D = decodeURIComponent(F[1]);
				if (!E[A]) {
					E[A] = []
				}
				E[A].push((F.length == 1) ? "" : D)
			}
			return E
		}
		;// var D=decodeURIComponent(F[1]);E[A]=((F.length==1)?"":D)}return E};
		...
		function SHARETHIS_merge() {
			...
			for ( var i = 0, l = arguments.length; i < l; i++) {
				...
				if (SHARETHIS_typeof(object) != 'object')
					continue;
				for ( var key in object) {
					var op = object[key], mp = mix[key];
					mix[key] = (mp && SHARETHIS_typeof(op) == 'object' && SHARETHIS_typeof(mp) == 'object') ? SHARETHIS_merge(
							mp, op)
							: SHARETHIS_unlink(op);
				}
			}
		...
		function SHARETHIS_unlink(object) {
			...
			case 'object':
				SHARETHIS_unlinked = {};
				for ( var p in object)
					SHARETHIS_unlinked[p] = SHARETHIS_unlink(object[p]);
				break;
			case 'hash':
			...
			case 'array':
				...
				for ( var i = 0, l = object.length; i < l; i++)
					SHARETHIS_unlinked[i] = SHARETHIS_unlink(object[i]);
				break;
			default:
				return object;
			}
		...
		function SHARETHIS_isArray(object) {
			var a = object != null && typeof object == "object"
		...
		function Shareable(properties, options) {
			...
			this.initialize = function(properties, options) {
				...
				if (options.target) {
					var o = this;
					options.target.onclick = function() {
						o.share();
					};
					if (options.mouseover) {
						options.target.onmouseover = function() {
							o.share();
						};
					}
				}
			...
			this.share = function() {
				frames['stframe'].location = this.frameUrl + "#getObject/"
						+ SHARETHIS.guid + "/" + this.idx;
			}
		...
		function ShareThis(options) {
			...
			this.sessionID_rand = Number(
					Math.random().toPrecision(5).toString().substr(2)).toString();
			...
			this.meta = {
				...
				location :location.pathname
					...
					this.addEntry = function(properties, options) {
						 ...
						 * document.body!=null){ console.log(document.body);
						 * console.log(typeof(document.body));
						 * document.write("FALE"); SHARETHIS.onReady(); }
						 */
						if (_thisScript === null) {
							var tmpScr = getShareThisScript();
							SHARETHIS.options = parseQueryString(tmpScr.src);
	
							this.frameUrl = "http://wd.sharethis.com/share3x/lightbox."
									+ STV + ".html";
							this.postUrl = "http://wd.sharethis.com/api/setCache_ws.php"
	
							if (SHARETHIS.options["button"]) {
								SHARETHIS.options["button"] = SHARETHIS
										.getBool(SHARETHIS.options["button"]
												.toString());
							}
							if (SHARETHIS.options["popup"]) {
								SHARETHIS.options["popup"] = SHARETHIS
										.getBool(SHARETHIS.options["popup"]
												.toString());
							}
							if (SHARETHIS.options["embeds"]) {
								SHARETHIS.options["embeds"] = SHARETHIS
										.getBool(SHARETHIS.options["embeds"]
												.toString());
							}
							...
							for ( var o in SHARETHIS.options) {
								if (SHARETHIS_tstOptions(o) == true) {
									...
									this.newwinfrag = this.newwinfrag + "/" + o
											+ "=" + encodeURIComponent(options[o]);
								}
							}
	
							SHARETHIS.initstr = init;
							SHARETHIS.mainstframe.src = SHARETHIS.frameUrl
									+ SHARETHIS.initstr;
						}
	
						var o = new Shareable(properties, SHARETHIS_merge(
								SHARETHIS.options, options));
						if (typeof (o.properties.url) === "object") {
							try {
								o.properties.url = o.properties.url.href
							} catch (err) {
							}
						}
						for ( var prop in o.properties) {
							try {
								o.properties[prop] = o.properties[prop].toString();
							} catch (err) {
							}
						}
						var xInt = "";
						...
						//
						if (this.meta.publisher == "65ab919d-0cc6-44bb-909e-e3db6f8dde10"
								...
								|| this.meta.publisher == "a5c90e3a-1cbd-44fc-a491-28f6729a38f4") { // temp
																									// change
																									// for
																									// on
																									// hover
						...
						o.chicklet = function(e) {
							...
							var loggerUrl = "http://l.sharethis.com/log?event=click"
									+ "&source=chicklet"
									+ "&publisher="
									+ encodeURIComponent(SHARETHIS.options.publisher)
									+ "&hostname="
									+ encodeURIComponent(SHARETHIS.meta.hostname)
									+ "&location="
									+ encodeURIComponent(SHARETHIS.meta.location)
									+ "&destinations="
									+ dest
									+ "&ts"
									+ (new Date()).getTime()
									+ "&title="
									+ encodeURIComponent(o.properties.title)
									+ "&url="
									+ encodeURIComponent(cleanURL(o.properties.url))
									+ "&sessionID="
									+ SHARETHIS.options.sessionID
							...
							url += "&p="
									+ encodeURIComponent(ST_JSON
											.encode(o.properties));
							window
									.open(url, "stpopup",
											"width=970,height=700,location=1,toolbar=1,scrollbars=1,menubar=1,resizable=1");
						}
						o.popup = function(e) {
							o.options.autoclose = true;
							SHARETHIS.postEntries(o); // manu
							// o.options.onmouseover=true;//setting to true for
							// default...
							if (SHARETHIS_TOOLBAR === true) {
								...
								SHARETHIS.mainstframe.src = SHARETHIS.frameUrl
										+ SHARETHIS.newwinfrag + "/guid_index=0"
										+ "/guid=" + SHARETHIS.guid + added_tool
										+ pgval;
								SHARETHIS.wrapper.style.visibility = "visible";
								SHARETHIS.mainstframe.style.visibility = 'visible';
							} else {
								if ((SHARETHIS.ready === true && SHARETHIS.frameReady === true)
										|| (SHARETHIS.popupExists === true
												&& SHARETHIS.ready == true && SHARETHIS.widgetExists === false)
										|| (SHARETHIS.popupExists === true
												&& SHARETHIS.ready == true && SHARETHIS.frameReady === true)) {
									clearInterval(stVisibleInterval);
									if ((typeof (e) != "undefined"
											&& typeof (e) != "unknown" && e)
											|| (typeof (event) != "undefined"
													&& typeof (event) != "unknown" && event)) {
										if (typeof (e) != "undefined"
												&& typeof (e) != "unknown" && e) {
											o.trigger = e.target
										} else if (typeof (event) != "undefined"
												&& typeof (event) != "unknown"
												&& event) {
									...
									if (SHARETHIS.curr_offsetTop > 0
											|| SHARETHIS.curr_offsetTop > 0) {
										SHARETHIS.autoPosition = false;
									}
									SHARETHIS.curr_id = id;
									if (o.options.onclick) {
										var res = o.options.onclick.apply(document,
												[ o ]);
										if (res == false)
											return false;
									}
									if (o.options.popup) {
										var newwinurl = SHARETHIS.frameUrl
												+ SHARETHIS.newwinfrag
												+ "/guid_index=0" + "/guid="
												+ SHARETHIS.guid + pageFrag;
										window
												.open(newwinurl, "newstframe",
														"status=1,toolbar=0,width=353,height=598");
									} else {
										if (st_showing == false) {
											...
											if (SHARETHIS.sendNum < SHARETHIS.sendArray.length) {
												SHARETHIS.sendArray.push("#show"
														+ "/guid_index=0"
														+ pageFrag);
												if (SHARETHIS.delayShow === true) {
													sendDataInt = setTimeout(
															SHARETHIS.sendData,
															1000);
												} else {
													sendDataInt = setTimeout(
															SHARETHIS.sendData, 20);
												}
											} else {
												// SHARETHIS.mainstframe.src =
												// SHARETHIS.frameUrl + "#show" +
												// "/guid_index=" + oidx;
												window.frames['stframe'].location
														.replace(SHARETHIS.frameUrl
																+ "#show"
																+ "/guid_index=0"
																+ pageFrag);
												if (SHARETHIS.delayShow === true) {
						...
						a.className = 'stbutton stico_' + (o.options.style ? o.options.style
								: (SHARETHIS.options.style ? SHARETHIS.options.style
										: 'default'));
						...
						if (o.options.onmouseover == false
								|| o.options.onmouseover == "false")
							a.onclick = o.popup;
						if (o.options.onmouseover == true
								|| o.options.onmouseover == "true") {
							SHARETHIS.wrapper.onmouseover = function() {
								stCancelClose();
							};
							a.onmouseover = function() {/*
														 ...
														 */
								stCancelClose();
								SHARETHIS.mousetimer = setTimeout(o.popup, 150);
							};
							a.onmouseout = function() {/*
														 * console.log("button mouse
														 * out");
														 */
								clearInterval(SHARETHIS.mousetimer);
								stClose();
							};
						...
						t
								.appendChild(document
										.createTextNode(o.options.buttonText));
						a.appendChild(t);
						o.button = a;
						try {
					...
					this.postEntries = function(o) {
						...
						var tmp = "/pageTitle="
								+ encodeURIComponent(encodeURIComponent(document.title))
								+ "/pageURL="
								+ encodeURIComponent(encodeURIComponent(cleanURL(document.URL)))
								+ "/pageHost="
								+ encodeURIComponent(encodeURIComponent(document.location.host))
								+ "/pagePath="
								+ encodeURIComponent(encodeURIComponent(document.location.pathname));
						...
						for ( var i = 0; i < sendArr.length; i++) {
							tmpSend = "#data/jsonData="
									+ encodeURIComponent(sendArr[i]);
					...
					this.sendJSON = function() {
						if (SHARETHIS.sendNum < SHARETHIS.sendArray.length) {
							...
							window.frames['stframe'].location
									.replace(SHARETHIS.frameUrl
											+ SHARETHIS.sendArray[SHARETHIS.sendNum]);
						} else {
							clearInterval(xInt);
					...
					this.manageHashTracking = function(url_str) {
						try {
							...
							if (match_results != null && match_results.length > 1) {
								...
								if (temp_arr != null) {
									this.referrer_sts = parseInt(temp_arr[0], 36)
											+ '.' + parseInt(temp_arr[1], 36);
									if (match_results.length > 2
											&& match_results[3] != null) {
										this.shr_flag = match_results[3];
									}
								}
							}
	
							if (this.hash_flag == true
									&& (match_results != null || url_str.split('#',
											2).length < 2)) {
					...
					this.log = function(event, obj, source) {
						...
						lurl += "&publisher="
								+ encodeURIComponent(SHARETHIS.meta.publisher)
								+ "&hostname="
								+ encodeURIComponent(SHARETHIS.meta.hostname)
								+ "&location="
								+ encodeURIComponent(SHARETHIS.meta.location)
								+ "&url=" + encodeURIComponent(cleanURL(url))
								+ "&sessionID=" + SHARETHIS.sessionID + "&fpc="
								+ SHARETHIS.fpc + "&ts" + (new Date()).getTime()
								+ "." + SHARETHIS.counter++ + "&r_sessionID="
								+ this.referrer_sts + "&hash_flag="
								+ SHARETHIS.hash_flag + "&shr=" + this.shr_flag;
	
						var logger2 = new Image(1, 1);
						logger2.src = lurl;
						// N.B. This onload function is required for IE.
						logger2.onload = function() {
							return;
						};
			...
			this.sendEvent = function(name, value) {
				...
				window.frames['stframe'].location.replace(SHARETHIS.frameUrl
						+ tmpSend);
			}
			...
			this.initialize = function(options) {
				...
				this.frameUrl = "http://wd.sharethis.com/share3x/lightbox." + STV
						+ ".html";
				this.postUrl = "http://wd.sharethis.com/api/setCache_ws.php"
				this.options = options || {};
				if (this.options["button"]) {
					this.options["button"] = this.getBool(this.options["button"]
							.toString());
				}
				if (this.options["popup"]) {
					this.options["popup"] = this.getBool(this.options["popup"]
							.toString());
				}
				if (this.options["embeds"]) {
					this.options["embeds"] = this.getBool(this.options["embeds"]
							.toString());
				}
				...
				try {
					this.mainstframe = document
							.createElement('<iframe name="stframe" allowTransparency="true" style="body{background:transparent;}" ></iframe>');
				...
				try {
					this.fp = document
							.createElement('<iframe name="stpostframe" style="visibility:hidden"></iframe>');
				} catch (err) {
				...
				this.defer( function() {
					// make button count call
						...
						burl += "&publisher="
								...
								+ "&hostname="
								+ encodeURIComponent(SHARETHIS.meta.hostname)
								+ "&location=" + "&url=" + "&sessionID="
								+ SHARETHIS.sessionID + "&fpc=" + SHARETHIS.fpc
								+ "&ts" + (new Date()).getTime() + "."
								+ SHARETHIS.counter++ + "&count="
								+ SHARETHIS.buttonCount + additional;
	
						var logger3 = new Image(1, 1);
						logger3.src = burl;
						logger3.onload = function() {
							return;
						};
						...
						if (SHARETHIS.popupExists === true
								&& SHARETHIS.popupCalled === false) {
							document.body.appendChild(SHARETHIS.fp);
							SHARETHIS.postPopup();
							SHARETHIS.popupCalled = true;
						}
						if (SHARETHIS.widgetCalled === false
								&& SHARETHIS.widgetExists === true) {
							document.body.appendChild(SHARETHIS.wrapper);
							try {
								window.frames['stframe'].location
										.replace(SHARETHIS.mainstframe.src);
							} catch (err) {
							}
							SHARETHIS.readyTestInterval = setInterval(
									SHARETHIS.readyTest, 250);
		...
		function stClose() {
			if (stautoclose == true)
				closetimeout = setTimeout("stcloseWidget()", 1500);
		}
		...
		function SHARETHIS_tstOptions(tstStr) {
			var opt_arr = [ 'type', 'title', 'summary', 'content', 'url', 'icon',
					'category', 'updated', 'published', 'author', 'button',
					'onmouseover', 'buttonText', 'popup', 'offsetLeft',
					'offsetTop', 'embeds', 'autoclose', 'publisher', 'tabs',
					'services', 'charset', 'headerbg', 'inactivebg', 'inactivefg',
					'linkfg', 'style', 'send_services', 'post_services',
					'headerfg', 'headerType', 'headerTitle', 'sessionID',
					'tracking', 'fpc', 'ads', 'pUrl', 'hash_flag' ];
			var retVal = false;
		...
		function _stFpc() {
			...
			if (cVal == false) {
				...
				if (cvArray.length == 4) {
					...
					num++;
					cv = cvArray[0] + "-" + cvArray[1] + "-" + cvArray[2] + "-"
							+ num;
					cVal = cv;
					_stSetFpc(cVal);
				}
		...
		function _stSetFpc(value) {
			...
			var exp_m = current_date.getMonth() + 9;// set cookie for 9 months into
													// future
		...
		function getShareThisScript() {
			...
			for ( var i = 0; i < _slist.length; i++) {
				...
				if (temp.search(/.*sharethis.*\/button/) >= 0) {
					rScript = _slist[i];
				} else if (temp.search(/.*sharethis.*\/widget\/\?/) >= 0
						|| temp.search(/.*sharethis.*\/widget\/index/) >= 0
						|| temp.search(/.*sharethis.*\/widget\/\?&/) >= 0) {
					rScript = _slist[i];
				}
			}
			return rScript;
		...
		function dbrInfo() {
			...
			if (dbr && dbr.length > 0) {
				...
				if (domain.length > 0) {
					retVal += "&refDomain=" + domain;
				} else {
					return false;
				}
				var query = dbr.replace(re2, "$2");
				if (query.length > 0) {
					retVal += "&refQuery=" + encodeURIComponent(query);
				}
				return retVal;
			} else {
				return false;
	...
	if (_thisScript2 && _thisScript2.parentNode.tagName != "HEAD"
			&& typeof (_sttoolbar) == "undefined") {
